...
if not exist "html\hls1935" mkdir "html\hls1935" >nul 2>&1
if not exist "html\dash1935" mkdir "html\dash1935" >nul 2>&1
...
netsh advfirewall firewall show rule name="NGINX-RTMP-HTTP-1935" >nul 2>&1
if %errorlevel% neq 0 (
    netsh advfirewall firewall add rule name="NGINX-RTMP-HTTP-1935" dir=in action=allow protocol=TCP localport=8080 >nul 2>&1
    if %errorlevel% equ 0 echo [OK] HTTP firewall rule added
)

netsh advfirewall firewall show rule name="NGINX-RTMP-RTMP-1935" >nul 2>&1
if %errorlevel% neq 0 (
    netsh advfirewall firewall add rule name="NGINX-RTMP-RTMP-1935" dir=in action=allow protocol=TCP localport=1935 >nul 2>&1
...
echo ========================================
echo [✓] RTMP Server: rtmp://localhost:1935/stream1935
echo [✓] HTTP Server: http://localhost:8080
echo [✓] Live Stream: http://localhost:8080/hls1935/stream1935.m3u8
...
powershell -Command "try { $response = Invoke-WebRequest -Uri 'http://localhost:8080/' -TimeoutSec 3 -UseBasicParsing -ErrorAction Stop; Write-Host '[OK] Local server responding' } catch { Write-Host '[WARNING] Local server check failed, but continuing...' }"
...
echo [RESTART] To restart everything: start_1935.bat
echo [STOP ONLY] To stop nginx only: stop_1935.bat
echo.
