...
if not exist "html\hls1941" mkdir "html\hls1941" >nul 2>&1
if not exist "html\dash1941" mkdir "html\dash1941" >nul 2>&1
...
netsh advfirewall firewall show rule name="NGINX-RTMP-HTTP-1941" >nul 2>&1
if %errorlevel% neq 0 (
    netsh advfirewall firewall add rule name="NGINX-RTMP-HTTP-1941" dir=in action=allow protocol=TCP localport=8086 >nul 2>&1
    if %errorlevel% equ 0 echo [OK] HTTP firewall rule added
)

netsh advfirewall firewall show rule name="NGINX-RTMP-RTMP-1941" >nul 2>&1
if %errorlevel% neq 0 (
    netsh advfirewall firewall add rule name="NGINX-RTMP-RTMP-1941" dir=in action=allow protocol=TCP localport=1941 >nul 2>&1
...
echo ========================================
echo [✓] RTMP Server: rtmp://localhost:1941/stream1941
echo [✓] HTTP Server: http://localhost:8086
echo [✓] Live Stream: http://localhost:8086/hls1941/stream1941.m3u8
echo [✓] Statistics: http://localhost:8086/stat
echo.
...
powershell -Command "try { $response = Invoke-WebRequest -Uri 'http://localhost:8086/' -TimeoutSec 3 -UseBasicParsing -ErrorAction Stop; Write-Host '[OK] Local server responding' } catch { Write-Host '[WARNING] Local server check failed, but continuing...' }"
...
.\cloudflared.exe tunnel --url localhost:8086
...
echo [RESTART] To restart everything: start_1941.bat
echo [STOP ONLY] To stop nginx only: stop_1941.bat
echo.
