...
if not exist "html\hls1937" mkdir "html\hls1937" >nul 2>&1
if not exist "html\dash1937" mkdir "html\dash1937" >nul 2>&1
...
netsh advfirewall firewall show rule name="NGINX-RTMP-HTTP-1937" >nul 2>&1
if %errorlevel% neq 0 (
    netsh advfirewall firewall add rule name="NGINX-RTMP-HTTP-1937" dir=in action=allow protocol=TCP localport=8082 >nul 2>&1
    if %errorlevel% equ 0 echo [OK] HTTP firewall rule added
)

netsh advfirewall firewall show rule name="NGINX-RTMP-RTMP-1937" >nul 2>&1
if %errorlevel% neq 0 (
    netsh advfirewall firewall add rule name="NGINX-RTMP-RTMP-1937" dir=in action=allow protocol=TCP localport=1937 >nul 2>&1
...
echo ========================================
echo [✓] RTMP Server: rtmp://localhost:1937/stream1937
echo [✓] HTTP Server: http://localhost:8082
echo [✓] Live Stream: http://localhost:8082/hls1937/stream1937.m3u8
echo [✓] Statistics: http://localhost:8082/stat
echo.
...
.\cloudflared.exe tunnel --url localhost:8082
