...
powershell -Command "try { Invoke-WebRequest -Uri 'http://localhost:8083/health' -TimeoutSec 5 -UseBasicParsing | Out-Null; Write-Host '[OK] Local server responding' } catch { Write-Host '[ERROR] Local server not responding'; exit 1 }"
if %errorlevel% neq 0 (
