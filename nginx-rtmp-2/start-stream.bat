...
title NGINX-RTMP Live Streaming #2 + Cloudflare Tunnel
color 0B
cd /d "%~dp0"

echo ========================================
echo   LIVE STREAMING SERVER #2 + PUBLIC TUNNEL
echo ========================================
echo.

REM Create directories for Stream #2
if not exist "html\hls2" mkdir "html\hls2" >nul 2>&1
if not exist "html\dash2" mkdir "html\dash2" >nul 2>&1
...
netsh advfirewall firewall show rule name="NGINX-RTMP-HTTP-2" >nul 2>&1
if %errorlevel% neq 0 (
    netsh advfirewall firewall add rule name="NGINX-RTMP-HTTP-2" dir=in action=allow protocol=TCP localport=8081 >nul 2>&1
    if %errorlevel% equ 0 echo [OK] HTTP firewall rule added (port 8081)
)

netsh advfirewall firewall show rule name="NGINX-RTMP-RTMP-2" >nul 2>&1
if %errorlevel% neq 0 (
    netsh advfirewall firewall add rule name="NGINX-RTMP-RTMP-2" dir=in action=allow protocol=TCP localport=1936 >nul 2>&1
...
echo.
echo ========================================
echo          LOCAL SERVER #2 READY
echo ========================================
echo [✓] RTMP Server: rtmp://localhost:1936/live2
echo [✓] HTTP Server: http://localhost:8081
echo [✓] Live Stream: http://localhost:8081/hls2/live2.m3u8
...
powershell -Command "try { Invoke-WebRequest -Uri 'http://localhost:8081/health' -TimeoutSec 5 -UseBasicParsing | Out-Null; Write-Host '[OK] Local server #2 responding' } catch { Write-Host '[ERROR] Local server #2 not responding'; exit 1 }"
if %errorlevel% neq 0 (
