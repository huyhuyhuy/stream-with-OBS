...
if not exist "html\hls1939" mkdir "html\hls1939" >nul 2>&1
if not exist "html\dash1939" mkdir "html\dash1939" >nul 2>&1
...
netsh advfirewall firewall show rule name="NGINX-RTMP-HTTP-1939" >nul 2>&1
if %errorlevel% neq 0 (
    netsh advfirewall firewall add rule name="NGINX-RTMP-HTTP-1939" dir=in action=allow protocol=TCP localport=8084 >nul 2>&1
    if %errorlevel% equ 0 echo [OK] HTTP firewall rule added
)

netsh advfirewall firewall show rule name="NGINX-RTMP-RTMP-1939" >nul 2>&1
if %errorlevel% neq 0 (
    netsh advfirewall firewall add rule name="NGINX-RTMP-RTMP-1939" dir=in action=allow protocol=TCP localport=1939 >nul 2>&1
...
echo ========================================
echo [✓] RTMP Server: rtmp://localhost:1939/stream1939
echo [✓] HTTP Server: http://localhost:8084
echo [✓] Live Stream: http://localhost:8084/hls1939/stream1939.m3u8
echo [✓] Statistics: http://localhost:8084/stat
echo.
...
powershell -Command "try { $response = Invoke-WebRequest -Uri 'http://localhost:8084/' -TimeoutSec 3 -UseBasicParsing -ErrorAction Stop; Write-Host '[OK] Local server responding' } catch { Write-Host '[WARNING] Local server check failed, but continuing...' }"
...
.\cloudflared.exe tunnel --url localhost:8084
...
echo [RESTART] To restart everything: start_1939.bat
echo [STOP ONLY] To stop nginx only: stop_1939.bat
echo.
