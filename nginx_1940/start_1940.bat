...
if not exist "html\hls1940" mkdir "html\hls1940" >nul 2>&1
if not exist "html\dash1940" mkdir "html\dash1940" >nul 2>&1
...
netsh advfirewall firewall show rule name="NGINX-RTMP-HTTP-1940" >nul 2>&1
if %errorlevel% neq 0 (
    netsh advfirewall firewall add rule name="NGINX-RTMP-HTTP-1940" dir=in action=allow protocol=TCP localport=8085 >nul 2>&1
    if %errorlevel% equ 0 echo [OK] HTTP firewall rule added
)

netsh advfirewall firewall show rule name="NGINX-RTMP-RTMP-1940" >nul 2>&1
if %errorlevel% neq 0 (
    netsh advfirewall firewall add rule name="NGINX-RTMP-RTMP-1940" dir=in action=allow protocol=TCP localport=1940 >nul 2>&1
...
echo ========================================
echo [✓] RTMP Server: rtmp://localhost:1940/stream1940
echo [✓] HTTP Server: http://localhost:8085
echo [✓] Live Stream: http://localhost:8085/hls1940/stream1940.m3u8
echo [✓] Statistics: http://localhost:8085/stat
echo.
...
powershell -Command "try { $response = Invoke-WebRequest -Uri 'http://localhost:8085/' -TimeoutSec 3 -UseBasicParsing -ErrorAction Stop; Write-Host '[OK] Local server responding' } catch { Write-Host '[WARNING] Local server check failed, but continuing...' }"
...
.\cloudflared.exe tunnel --url localhost:8085
...
echo [RESTART] To restart everything: start_1940.bat
echo [STOP ONLY] To stop nginx only: stop_1940.bat
echo.
